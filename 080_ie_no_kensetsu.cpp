...
int main() {
    ...
    vector<vector<ll>> agg(H+1, vector<ll>(W+1));
    REP1(i, H) {
        REP1(j, W) {
            agg[i][j] = v[i][j] + agg[i-1][j] + agg[i][j-1] - agg[i-1][j-1];
        }
    }
    ...
    for (int a = 1; a < H+1; a++) {
        for (int b = 1; b < W+1; b++) {
            for (int c = a; c < H+1; c++) {
                for (int d = b; d < W+1; d++) {
                    ll land = agg[c][d] - agg[c][b-1] - agg[a-1][d] + agg[a-1][b-1];
                    ll size = (c-a+1)*(d-b+1);
                    ll house = K * size; 
                    ll cost = land + house;
                    if (cost <= V) ans = max(ans, size);
                }
            }
        }
    }
