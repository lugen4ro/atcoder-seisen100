...
int main() {
    int N,M;
    cin>>N>>M;
    vector<vector<int>> imos(N+2, vector<int>(N+2)); // 0-indexed with 2 buffer 
    REP(i, M) {
        int a,b,x;
        cin>>a>>b>>x;
        a--;b--; // to 0-indexed

        // Top point and bottom border and compensation for first left->right 
        imos[a][b]++;
        imos[a][b+1]--;
        imos[a+x+1][b]--;
        imos[a+x+1][b+x+2]++;

        // Compensation for top->bottom-left for bottom points
        imos[a+x+2][b+1]++;
        imos[a+x+2][b+x+2]--;
    }
