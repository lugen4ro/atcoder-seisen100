...
int main() {
    int N,M; // 1000
    cin>>N>>M;
    vector<int> hirou(N+1);
    vector<int> tenkou(M+1);
    REP(i, N) cin>>hirou[i+1];
    REP(i, M) cin>>tenkou[i+1];
    ...
    REP1(i, M) { // day
        REP(j, N+1) { // town
            // Already there last day OR moved on day i
            dp[i][j] = min(dp[i-1][j], dp[i-1][j-1] + hirou[j]*tenkou[i]);
        }
    }
    PRINT(dp[M][N]);
}
