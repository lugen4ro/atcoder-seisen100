...
int main() {
    ...
    REP1(i, M) REP1(j, N) cin >> v[i][j];
    vector<vector<int>> queries(K, vector<int>(4));
    VIN2(queries, K, 4);

    // Create prefix-sums (aggregate sums)
    vector<vector<vector<int>>> agg(M+1, vector<vector<int>>(N+1, vector<int>(3)));
    REP1(i, M) {
        REP1(j, N) {
            vector<int> t(3);
            if (v[i][j] == 'J') t[0] = 1;
            else if (v[i][j] == 'O') t[1] = 1;
            else if (v[i][j] == 'I') t[2] = 1;

            REP(l, 3) {
                agg[i][j][l] = t[l] + agg[i-1][j][l] + agg[i][j-1][l] - agg[i-1][j-1][l];
            }
        }
    }

    // Create answer for each query
    REP(i, K) {
        int a = queries[i][0];
        int b = queries[i][1];
        int c = queries[i][2];
        ...
        REP(l, 3) {
            cout << agg[c][d][l] - agg[c][b-1][l] - agg[a-1][d][l] + agg[a-1][b-1][l];
            if (l<2) cout << " ";
        }
        cout << endl;
    }
