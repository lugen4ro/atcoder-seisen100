...
int main() {
    ...
    vector<vector<ll>> dp(1<<V, vector<ll>(V, LINF));
    dp[0][0] = 0; // Only valid source to come from is 0, and there is no vertice travelled to yet so S = 0
    ...
    REP(S, 1<<V) {
        ...
        REP(v, V) {
            REP(u, V) {
                ...
                chmin(dp[S|1<<v][v], dp[S][u] + cost[u][v]);
            }
        }
    }

    // Visited all, and last was 0
    if (dp[(1<<V)-1][0] == LINF) cout << -1 << endl;
