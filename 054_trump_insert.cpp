...
int main() {
    ...
    V<int> dp(N+1, INF); // dp[i] := minimum value that ends the Increasing-Subsequence of length i
    dp[0] = -1;
    REP(i, N) {
        auto smaller = lower_bound(dp.begin(), dp.begin()+i+1, v[i]) - 1;
        int length = smaller-dp.begin();
        chmin(dp[length+1], v[i]);
    }
    int lis = 0;
    RREP(i, N+1) if (dp[i] != INF) {lis = i; break;}
    PRINT(N-lis);
