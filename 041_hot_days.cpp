...
int main() {
    ...
    REP(i, D) {
        REP(j, N) {
            ...
            // First day
            if (i==0) if (BC(days[i], a, b+1)) {dp[i][j] = 0; continue;}

            // Can wear shirt on day i
            if (BC(days[i], a, b+1)) {
                // Set max score for shirt j on day i
                REP(k, N) {
                    dp[i][j] = max(dp[i][j], dp[i-1][k] + abs(c-get<2>(v[k])));
                }
            }
        }
    }
    PRINT(*max_element(ALL(dp[D-1])));
}
