...
int main() {
    ...
    V<int> dp(n+1, INF); // dp[i] has value of minimum ending value of subsequence of length i
    dp[0] = -1;
    REP(i, n) {
        int cur = v[i];
        auto lb = lower_bound(dp.begin(), dp.begin()+i+1, cur) - 1; // Smaller than cur
        int length = lb - dp.begin();
        chmin(dp[length+1], cur);
    }

    int ans=0;
    RREP(i, n+1) if (dp[i] != INF){ ans = i; break; }
    PRINT(ans);
}
